...
const Input = (props: InputFile) => {
  ...
  return (
    <props.Filed name={props.name}
          validators={{
            onChange : ({value}) => (
                !value ? props.error : undefined
            ),
          ...
          children={(filed) => ( 
            <div className={props.classParent}>
                <input type={props.type} className={props.className} onClick={HandleClick} name={filed.name} defaultValue={props.value || undefined} value={filed.state.value} onChange={(e) => filed.handleChange(e.target.value)} placeholder={props.placeholder} autoComplete="on" />
